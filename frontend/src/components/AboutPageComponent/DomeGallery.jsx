...
function buildItems(pool, seg) {
  ...
  // Extended Y range for more rows at top and bottom (fuller globe)
...
export default function DomeGallery({
  ...
  useEffect(() => {
    ...
    const step = (now) => {
      ...
      const prev = lastAutoRotateTime.current;
      lastAutoRotateTime.current = now;
      if (draggingRef.current || inertiaRAF.current || hoveringRef.current) return;
      const deltaMs = prev != null ? now - prev : 0;
      const deltaDeg = (autoRotateSpeed * deltaMs) / 1000;
  ...
  const openItemFromElement = useCallback(
    (el) => {
      if (openingRef.current || !el) return;
  ...
  const onTileMouseLeave = useCallback(() => {
    ...
    hoveringRef.current = hoverCountRef.current > 0;
  }, []);
  ...
  if (items.length === 0 || items.every((it) => !it.src)) {
    return null;
  }
