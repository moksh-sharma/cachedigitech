...
function Certifications() {
  ...
  return (
    <div className="w-full min-h-screen bg-[#fafafa]">
      {/* Logos / Awards and Recognitions — matches Framer Logos section style */}
      <section
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-5 sm:gap-6">
            {certifications.map((cert) => (
              ...
              >
                ...
                <span className="text-xs sm:text-sm font-medium text-[var(--apple-black)] text-center leading-tight group-hover:text-red-600 transition-colors">
                  {cert.name}
                </span>
                {cert.description && (
                  <span className="mt-1 text-[11px] sm:text-xs text-[var(--apple-gray)] text-center leading-snug line-clamp-2">
                    {cert.description}
                  </span>
                )}
              </div>
